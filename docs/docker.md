...
- Nothing inside the container is scheduled. If you use automation
  Wait steps, flows, or the Dropi orders integration, point an
  external scheduler at `GET /api/automations/cron`,
  `GET /api/flows/cron`, and `GET /api/dropi/cron` on this deployment,
  sending the shared secret in the `x-cron-secret` header
  (`AUTOMATION_CRON_SECRET`, see `.env.local.example`). All three
  return 503 until that variable is set. For `/api/dropi/cron`, every
  5-10 minutes is a reasonable interval — it's a no-op for any account
  that hasn't turned Dropi sync on.
